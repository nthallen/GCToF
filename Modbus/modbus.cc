...
modbus::modbus(const char *path, Modbus_t *MB_TM) :
  ...
  if (opt_f4) {
    ...
    polls.push_back(new modbus_poll_int(F4_1_ID, 108, &MB_TM->F4_T3));
  ...
  if (flag & (Selector::Sel_Read | Selector::Sel_Timeout)) {
    ...
    if (pending) {
      ...
      if (cp + echo_sz + pending->rep_sz > nc) {
        ...
        switch (pending->process_reply(&buf[cp+echo_sz], nc-cp-echo_sz)) {
          case MB_rep_ok:
            report_ok();
            break;
 ...
 */
modbus_cmd::modbus_cmd(modbus *MB_in) : Ser_Sel() {
  char shortname[40];
  int nc;
  nc = snprintf(shortname, 40, "cmd/%s", modbus_name);
  if (nc >= 40) {
    nl_error(3, "Specified name '%s' is too long", modbus_name);
  }
  init( tm_dev_name(shortname), O_RDONLY|O_NONBLOCK, 40 );
  MB = MB_in;
}
